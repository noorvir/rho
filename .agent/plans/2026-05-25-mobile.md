...
Create a separate mobile package under `packages/mobile` with package name `@rho/mobile`. Keep it independent from `@rho/cli` so CLI installs do not pull React Native or Expo dependencies.
...
- `cd packages/mobile && bun run dev` starts Expo.
...
- [x] Add minimal Expo package/config.
- [x] Add root layout, placeholder routes, bottom nav, and global chat shell.
- [x] Validate typecheck scripts and Expo startup instructions.
- [ ] Verify chat interactions with Maestro once Java/Maestro MCP is available.
