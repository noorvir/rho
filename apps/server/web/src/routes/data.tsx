...
export function DataPage({ tableName }: { tableName?: string }) {
  const tablesQuery = useQuery(orpc.data.listTables.queryOptions());
  const tableQuery = useQuery(
    orpc.data.table.queryOptions({
      input: tableName ? { name: tableName } : skipToken,
  ...
  const selectedTable = tables.find(
    (nextTable) => nextTable.name === tableName
  );
  const columns = useMemo(
    () => (table ? dataColumns(table.columns) : []),
    [table]
  );
      ...
      <div className="flex min-h-0 flex-1">
        <aside className="flex w-52 shrink-0 flex-col border-r border-border">
          ...
          <div className="scrollbar-thin min-h-0 flex-1 overflow-auto p-2">
            {tablesError ? (
              <p className="p-2 text-xs text-destructive">{tablesError}</p>
            ) : null}
    ...
    <>
      <div className="flex h-10 items-center justify-between gap-3 px-4">
        <DataTableChip>
          ...
          <ChipSeparator />
          <button
            className="inline-flex h-full items-center gap-1 px-2"
            type="button"
          >
            {rows.length} rows
    ...
    <div className="flex min-h-0 flex-1 items-center justify-center p-6 text-center">
      <div>
        ...
        <p className="mt-1 max-w-xs text-sm text-muted-foreground">
          Select a SQLite table from the Data list to inspect its columns and
          rows.
        </p>
      </div>
    </div>
